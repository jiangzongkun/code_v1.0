...
Key rules:
- PICK only if your gripper is empty (holding nothing).
- PLACE only if you are already holding the item you want to place.
- Do NOT PICK an item already inside the bin (shown as "inside slot X").
- Do NOT PLACE into a slot already occupied by another item (shown as "occupied").
- Do NOT PICK the same item as the other robot in the same round.
- Prefer one active robot per round. The non-active robot should WAIT with 4 identical coords at its current position.
"""

PACK_ACTION_SPACE="""
[Action Options]
1) PICK <obj> PATH <path>: only PICK if your gripper is empty; do NOT PICK items already inside the bin.
2) PLACE <obj> <bin_slot> PATH <path>: only if you are already holding the object; choose an EMPTY slot (not occupied).
3) WAIT PATH <path>: stay still; use exactly 4 identical coords at your current gripper position.
...
The robot PATHs must efficiently reach target while avoiding collision (e.g. move above the objects' heights).
...
class PackGroceryTask(MujocoSimEnv):
    ...
    def _get_slot_occupant(self, obs: EnvState, slot_name: str) -> Optional[str]:
        """Return item name currently occupying this bin slot, or None if empty."""
        slot_xy = self.bin_slot_xposes[slot_name][:2]
        for item_name in self.item_names:
            item_state = obs.objects.get(item_name)
            if item_state is None:
                continue
            if 'bin_inside' not in item_state.contacts:
                continue
            item_xy = np.asarray(item_state.xpos[:2], dtype=float)
            if np.linalg.norm(item_xy - slot_xy) < 0.12:
                return item_name
        return None
    ...
    def describe_object(self, obs, name):
        ...
        if 'bin_inside' in contacts:
            ...
            object_desp += f"inside slot {slot_name} [ALREADY PACKED - do NOT pick again]"
        else:
            object_desp += f"on table [needs to be packed]"
        return object_desp
    ...
    def describe_obs(self, obs: EnvState):
        ...
        for name in self.item_names:
            if 'bin_inside' in obs.objects[name].contacts:
                packed_items.append(name)
            else:
                unpacked_items.append(name)
        packed_str = ', '.join(packed_items) if packed_items else 'none'
        unpacked_str = ', '.join(unpacked_items) if unpacked_items else 'ALL DONE'
        full_desp += (
            f"[Task Progress] {len(packed_items)}/{len(self.item_names)} packed. "
            f"Already packed: {packed_str}. "
            f"Still on table: {unpacked_str}.\n"
        )

        # Robot hand status.
        for robot_name, agent_name in self.robot_name_map.items():
            robot_state = getattr(obs, robot_name)
            hand_contacts = [c for c in robot_state.contacts if c in self.item_names]
            holding = hand_contacts[0] if hand_contacts else "nothing (gripper empty)"
            full_desp += f"[{agent_name} hand status] holding: {holding}\n"

        # Per-object details.
        for name in self.item_names:
            full_desp += self.describe_object(obs, name) + "\n"

        # Bin slots with occupancy.
        for slot_name, slot_xpos in self.bin_slot_xposes.items():
            x, y, z = slot_xpos
            occupant = self._get_slot_occupant(obs, slot_name)
            slot_status = f"occupied by {occupant}" if occupant else "empty"
            full_desp += f"{slot_name}: ({x:.2f}, {y:.2f}, {z:.2f}) [{slot_status}]\n"
    ...
    def get_agent_prompt(self, obs, agent_name):
        ...
        # Show which items are still unpacked for clarity
        unpacked = [n for n in self.item_names if 'bin_inside' not in obs.objects[n].contacts]
        unpacked_str = ', '.join(unpacked) if unpacked else 'ALL DONE'
...
Items still needing to be packed: {unpacked_str}
...
"""
    ...
    def get_task_feedback(self, llm_plan, pose_dict):
        """Validate Pack actions against current environment state."""
        obs = self.get_obs()
        feedback = ""
        items_being_picked = {}  # item -> agent_name, to detect duplicate picks

        for agent_name, action_str in llm_plan.action_strs.items():
            robot_name = self.robot_name_map_inv[agent_name]
            robot_state = getattr(obs, robot_name)
            hand_contacts = [c for c in robot_state.contacts if c in self.item_names]
            holding = hand_contacts[0] if hand_contacts else None

            # Basic action type check.
            has_pick = 'PICK' in action_str
            has_place = 'PLACE' in action_str
            has_wait = 'WAIT' in action_str
            if not has_pick and not has_place and not has_wait:
                feedback += (
                    f"{agent_name}'s ACTION is invalid: must be PICK, PLACE, or WAIT. "
                )
                continue
            ...
            if has_pick and not has_place:
                ...
                if holding is not None:
                    feedback += (
                        f"{agent_name} cannot PICK: already holding {holding}. "
                        f"Must PLACE {holding} into an empty bin slot first. "
                    )
                else:
                    pick_idx = parts.index('PICK') if 'PICK' in parts else -1
                    if pick_idx >= 0 and pick_idx + 1 < len(parts):
                        item = parts[pick_idx + 1]
                        if item in self.item_names:
                            item_contacts = obs.objects[item].contacts if item in obs.objects else []
                            if 'bin_inside' in item_contacts:
                                feedback += (
                                    f"{agent_name} cannot PICK {item}: it is already packed inside the bin. "
                                    f"Choose an unpacked item instead. "
                                )
                            elif item in items_being_picked:
                                feedback += (
                                    f"{agent_name} cannot PICK {item}: "
                                    f"{items_being_picked[item]} is already picking it this round. "
                                    f"Pick a different unpacked item. "
                                )
                            else:
                                items_being_picked[item] = agent_name

            elif has_place:
                # PLACE validation.
                if holding is None:
                    feedback += (
                        f"{agent_name} cannot PLACE: gripper is empty. "
                        f"Must PICK an item first. "
                    )
                else:
                    # Check the named slot is not occupied
                    slot_names = list(self.bin_slot_xposes.keys())
                    action_slots = [s for s in slot_names if s in action_str]
                    if action_slots:
                        slot = action_slots[0]
                        occupant = self._get_slot_occupant(obs, slot)
                        if occupant is not None and occupant != holding:
                            empty_slots = [
                                s for s in slot_names
                                if self._get_slot_occupant(obs, s) is None
                            ]
                            suggestion = (
                                f"Try: {empty_slots[0]}" if empty_slots else "No empty slots available"
                            )
                            feedback += (
                                f"{agent_name} cannot PLACE into {slot}: "
                                f"already occupied by {occupant}. {suggestion}. "
                            )
                    else:
                        feedback += (
                            f"{agent_name} PLACE action missing a valid bin slot name. "
                            f"Valid slots: {', '.join(slot_names)}. "
                        )
